(function() {
	$.fn.weixinAudio = function(options) {
		...
		Plugin.prototype = {
			...
			play: function() {
                ...
                $.each(obj,function(index,element){
                	if(obj[index].index!=self.index){
                        obj[index].Audio.pause();
                        obj[index].currentState = "pause";
                        clearInterval(obj[index].timer);
                        obj[index].$audio_area.removeClass('playing');
					}
				});
		...
		$this.each(function(index,element){
			obj['weixinAudio'+index] = new Plugin($(this),index);
		}); //多个执行返回对象
